...
function checkEmptyString() {
   ...
   if (!myInput.length) {
      ...
      document.getElementById('divResults').innerHTML =
         myInput + ' is not an empty string.'
   }
...
function invertString() {
   ...
   document.getElementById('divResults').innerHTML =
      myInput + ' in reverse is:<br>' + reverseInput
...
function countAs() {
   ...
   document.getElementById('divResults').innerHTML =
      ' The letter a is this many times in ' + myInput + ': ' + count
}
...
function findAs() {
   ...
   if (positionAFirst == -1) {
      ...
      if (positionALast == -positionAFirst) {
         document.getElementById('divResults').innerHTML =
            "This string only has one a, and it's at position " + positionAFirst
      } else if (positionAFirst == -1) {
         //eso seria lo mismo q lo anterior?
      } else {
         document.getElementById('divResults').innerHTML =
            'The first a is at position ' +
            positionAFirst +
            '<br> The last a is at position ' +
            positionALast +
            '<br>'
      }
   }
...
function validateEmail() {
   ...
   if (myInput.indexOf('@') == -1) {
      ...
      if (emailSplit.length < 2) {
         ...
         emailSplit[1].endsWith('.net') &&
         emailSplit[1].endsWith('.org') &&
         emailSplit[1].endsWith('.com')
      ) {
         message = 'THIS EMAIL IS NOT VALID: THERE IS NO EXTENSION AFTER @.'
      } else if (
         emailSplit[0].includes('.net') ||
         emailSplit[0].includes('.org') ||
         emailSplit[0].includes('.com')
      ) {
...
function tryValidateEmail(text) {
   ...
   function teUnaSolaArroba(text) {
      ...
      if (text.slice(text.indexOf('@') + 1).indexOf('@') != -1) return false
      return true
   }
   ...
   function extensio3caracters(text) {
      if (text.substr(-4, 1) == '.') return false
      return true
   }
   ...
   if (!extensioCorrecta(text)) return 'MAIL INVALID: extensió incorrecta'
